...
def main():
    ...
    optimizer = init_optimizer(
        args.optim, model.parameters(), args.lr, args.weight_decay)
    if use_gpu:
        model = model.cuda()
    ...
    for epoch in range(args.max_epoch):
        ...
        train(epoch, model, criterion, optimizer,
              trainloader, learning_rate, use_gpu)
        ...
        if epoch == 0 or epoch > (args.stepsize[0]-1) or (epoch + 1) % 10 == 0:
            ...
            print(
                "==> Test 5-way Best accuracy {:.2%}, achieved at epoch {}".format(best_acc, best_epoch))
...
def train(epoch, model, criterion, optimizer, trainloader, learning_rate, use_gpu):
    ...
    for batch_idx, (images_train, labels_train, images_test, labels_test, pids) in enumerate(trainloader):
        ...
        ytest = model(images_train, images_test,
